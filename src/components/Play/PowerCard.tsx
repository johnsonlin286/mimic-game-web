...
export default function PowerCard({ power }: PowerCardProps) {
  ...
  return (
    <div className="flex justify-center items-center w-full h-full">
      <div role="button" {...attrs} className={`relative w-full h-full md:max-w-80 md:max-h-[480px] aspect-3/4 perspective-1000 transform-3d transition-all duration-300 cursor-pointer ${isFlipped ? 'rotate-y-180' : ''}`}>
        <div className="w-full h-full transform-3d">
          <div className="absolute flex flex-col justify-between items-center gap-2 w-full h-full backface-hidden bg-dark-navy rounded-2xl shadow-lg p-6">
            <div className="flex-1 flex flex-col justify-center items-center gap-2">
              <h1 className="text-2xl md:text-3xl font-bold text-center text-white uppercase">{power.name}</h1>
              <p className="text-sm md:text-lg text-white text-center">{power.description}</p>
            </div>
            {power.type === 'active' && (
              <Button variant="primary" size="md" disabled={power.isUsed} onClick={() => handleUsePower(power.name)} className="w-full self-end">
          ...
          <div className="absolute flex flex-col justify-between items-center w-full h-full backface-hidden rotate-y-180 bg-dark-navy rounded-2xl shadow-lg p-6">
            <div className="flex-1 flex flex-col gap-3 justify-center items-center">
              <Image src={`${IMAGE_ASSETS_URL}/images/power-card.webp`} alt="morf-blank" width={0} height={0} sizes="100vw" className="w-40 h-auto md:w-60 md:h-auto pointer-events-none"/>
              <p className="text-sm md:text-lg text-center text-white font-fredoka">
